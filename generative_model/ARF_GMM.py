...
class arfgmm:
  ...
  def forde(self, dist="gmm", oob=False, alpha=0):
        ...
        :param dist: Distribution to use for density estimation of continuous features.
                     Distributions implemented so far: "truncnorm", defaults to "truncnorm"
        ...
        :param alpha: Optional pseudocount for Laplace smoothing of categorical features.
                      This avoids zero-mass points when test data fall outside the support of training data.
                      Effectively parametrizes a flat Dirichlet prior on multinomial likelihoods, defaults to 0
        ...
        if np.invert(self.factor_cols).any():
            for tree in range(self.num_trees):
                ...
                # Fit GMM to the data within each node
                gmm_params = []
                for (t, n, var), group in long.groupby(['tree', 'nodeid', 'variable']):
                    gmm = GaussianMixture(n_components=3)
                    gmm.fit(group['value'].values.reshape(-1, 1))
                    for i in range(gmm.n_components):
                        gmm_params.append([t, n, var, gmm.weights_[i], gmm.means_[i, 0], np.sqrt(gmm.covariances_[i, 0, 0])])
                
                res = pd.DataFrame(gmm_params, columns=['tree', 'nodeid', 'variable', 'weight', 'mean', 'sd'])
        ...
        return {"cnt": self.params, "cat": self.class_probs, "forest": self.clf, "meta": pd.DataFrame(data={"variable": self.orig_colnames, "family": self.dist})}# TO DO: optional -- think of dropping f_idx
  def forge(self, n):
    ...
    for j in range(self.p):
        ...
        if self.factor_cols[j]:
            ...
            # Continuous columns: Sample from GMM
            variable = obs_params[obs_params["variable"] == colname]
            gmm = GaussianMixture(n_components=3)  # Assuming 3 components as in your `forde` method
            gmm.weights_ = variable["weight"].values
