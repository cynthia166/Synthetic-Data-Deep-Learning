...
class arf:
  ...
  def forde(self, dist = "truncnorm", oob = False, alpha = 0):
    ...
    global_min = self.x_real.loc[:, np.invert(self.factor_cols)].min()
    global_max = self.x_real.loc[:, np.invert(self.factor_cols)].max()
...
"bnds": self.bnds,  # DataFrame con 'tree', 'nodeid' y 'cvg'
    "num_trees": self.num_trees,  # Número de árboles
    "params": self.params,  # DataFrame con parámetros de distribuciones
    "class_probs": self.class_probs,  # DataFrame con probabilidades de clases
    "p": self.p,  # Número de variables
    "orig_colnames": self.orig_colnames,  # Lista de nombres originales de columnas
    "factor_cols": self.factor_cols,  # Lista o Series de booleanos indicando columnas categóricas
    "object_cols": self.object_cols,  # Lista o Series de booleanos indicando columnas de tipo objeto
    "levels": self.levels,  # Diccionario mapeando nombres de columnas categóricas a sus niveles
    "dist": 'truncnorm'                                                            })}
