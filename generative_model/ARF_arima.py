...
def forde(self):
    # ... [Previous code remains the same up to the point where we fit distributions] ...
    pred = self.clf.apply(self.x_real.drop(columns=[self.subject_id_col]))
    # Fit continuous distribution and autoregressive models in all terminal nodes
    ...
    if np.invert(self.factor_cols).any():
        for tree in range(self.num_trees):
            ...
            long = pd.merge(right=self.bnds[['tree', 'nodeid','variable', 'min', 'max', 'f_idx']], 
                            left=pd.melt(dt[dt["nodeid"] >= 0], id_vars=["tree", "nodeid"]), 
                            on=['tree', 'nodeid', 'variable'], how='left')
            
            # Get distribution parameters and fit autoregressive models
            for (tree, nodeid, variable), group in long.groupby(['tree', 'nodeid', 'variable']):
                if len(group) > self.min_node_size:
                    # Fit distribution parameters (assuming you have a method for this)
                    dist_params = self._fit_distribution(group['value'])
                    self.params = pd.concat([self.params, pd.DataFrame({
                        'tree': [tree], 'nodeid': [nodeid], 'variable': [variable],
                        'dist_params': [dist_params]
                    })])
                    
                    # Fit autoregressive model
                    ar_model = sm.tsa.AutoReg(group['value'], lags=min(5, len(group) - 1)).fit()
                    if tree not in self.ar_models:
                        self.ar_models[tree] = {}
                    if nodeid not in self.ar_models[tree]:
                        self.ar_models[tree][nodeid] = {}
                    self.ar_models[tree][nodeid][variable] = ar_model
    ...
    if self.factor_cols.any():
        for tree in range(self.num_trees):
            ...
            probs = long.groupby(['tree', 'nodeid', 'variable', 'value']).size().unstack(fill_value=0)
            probs = probs.div(probs.sum(axis=1), axis=0)
            probs = probs.reset_index().melt(id_vars=['tree', 'nodeid', 'variable'], 
                                             var_name='category', value_name='probability')
            self.class_probs = pd.concat([self.class_probs, probs])



    return {"par_models": self.par_models, "forest": self.clf, "ar_models": self.ar_models, "class_probs": self.class_probs}

    def _train_leaf_par(self, leaf_data):
        sequences = []
        for subject_id, group in leaf_data.groupby(self.subject_id_col):
            sequence = {
                'context': [],
                'data': [group[col].tolist() for col in self.orig_colnames]
            }
            sequences.append(sequence)
        
        data_types = ['categorical' if self.factor_cols[col] else 'continuous' for col in self.orig_colnames]
...
# Usage
train_arf = True
save_path_arf = "/home-local2/cyyba.extra.nobkp/Synthetic-Data-Deep-Learning/generative_input/arf_arima.pkl"
train_data_features = load_data("/home-local2/cyyba.extra.nobkp/Synthetic-Data-Deep-Learning/generative_input/entire_ceros_tabular_data_accumulative_features.pkl")[:10000]
...
if train_arf:
    ...
    save_pickle(subject_arf, save_path_arf) 
else: 
    subject_arf = load_data(save_path_arf)
