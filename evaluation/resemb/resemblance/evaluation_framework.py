...
class EvaluationFramework:
    ...
    def anomaly_detection(
        ...
        for key in self.d_synthetic_data:
            ...
            def drop_timestamp_cols(df):
                # Identificar columnas de tipo Timestamp
                timestamp_cols = df.select_dtypes(include=[pd.Timestamp]).columns
                # Eliminar columnas de tipo Timestamp
                df = df.drop(columns=["ADMITTIME"])
                return df

            # Antes de ajustar el modelo, elimina las columnas de tipo Timestamp de df_real_data
            try:
              self.df_real_data = drop_timestamp_cols(self.df_real_data)
            except Exception as e:
              print(e)  
              pass
                  
            # Eliminar columnas de tipo Timestamp de cada DataFrame en d_synthetic_data
            for key in self.d_synthetic_data:
                try:
                    self.d_synthetic_data[key] = drop_timestamp_cols(self.d_synthetic_data[key])
                except Exception as e:
                    print(e)
                    pass
